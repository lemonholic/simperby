...
mod test {
    ...
    async fn broadcast_once() {
        ...
        sleep(create_testnet_config().broadcast_interval).await;
    ...
    async fn broadcast_large_growing_network() {
        ...
        network.create_nodes(n);
        network
            .add_nodes_to_network_concurrent((n / 2) as usize + 1)
            .await
            .expect("failed to add node to the network.");
        ...
        network
            .add_nodes_to_network_concurrent(5)
            .await
            .expect("failed to add node to the network.");
    ...
    async fn broadcast_get_status() {
        ...
        let keys: Vec<_> = network.nodes.keys().take(n).copied().collect();
        assert!(keys.len() == n, "not enough nodes");
        broadcast_with_parameters(&mut network, keys.to_owned(), 3, 100)
            .await
            .expect("failed to broadcast");

        // Wait for a moment before stopping.
        sleep(config.broadcast_interval).await;
        ...
        // Wait for the intervals of peer discovery and broadcasting.
        sleep(config.peer_discovery_interval.mul_f32(2.0) + config.broadcast_interval.mul_f32(2.0))
            .await;

        for key in stop_broadcast_keys.iter() {
            let expected = stop_broadcast_keys
                .iter()
                .chain(continue_broadcast_keys.iter())
